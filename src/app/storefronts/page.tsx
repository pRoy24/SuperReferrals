import { ExternalLink, Film, Search, ShieldCheck, SlidersHorizontal, Star, Store, Wallet } from "lucide-react";
import {
...
export default async function StorefrontDirectoryPage() {
  ...
  const storefronts = store.customers.map((customer) => buildStorefrontDirectoryItem(store, customer));
  ...
  return (
    <main className="public-main storefront-directory">
      ...
      <section className="storefront-search-strip">
        <Search size={18} />
        <span>{storefronts.length} created storefront{storefronts.length === 1 ? "" : "s"} available</span>
      </section>
...
type StorefrontDirectoryItem = ReturnType<typeof buildStorefrontDirectoryItem>;
