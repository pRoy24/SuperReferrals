...
function main() {
  ...
  const blobConfig = {
    ...
    storeName: options.storeName || config.vercel?.blob?.storeName || "superstores-private",
  ...
  const redisConfig = {
    ...
    resourceName: options.redisName || config.vercel?.redis?.resourceName || "superstores-redis",
...
function printHeader({ targetName, target, scope, project, blobConfig, redisConfig, dryRun }) {
  console.log(`${dryRun ? "Dry run: " : ""}SuperStores deploy bootstrap`);
...
function reuseRedisEnvFromTarget({ config, sourceTargetName, target, targetEnvKeys = new Set(), token, options }) {
  ...
  const dir = mkdtempSync(path.join(os.tmpdir(), "superstores-vercel-env-"));
...
function createMergedEnvFileForSync(target) {
  ...
  const variables = loadTargetEnvVariables(target);
  const dir = mkdtempSync(path.join(os.tmpdir(), "superstores-env-"));
