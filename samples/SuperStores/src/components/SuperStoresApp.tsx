...
export function SuperStoresApp() {
  ...
  const visibleListings = useMemo(() => {
    ...
    return listings.filter((listing) => {
      ...
      return `${listing.title} ${listing.description} ${collectionModeLabel(listing.collectionMode)} ${standardLabel(listing.tokenStandard)} ${saleMechanismLabel(listing.saleMechanism)} ${metadataSummary(listing)}`
        .toLowerCase()
        .includes(needle);
    });
  }, [currencyFilter, listings, metadataFilter, typeFilters]);
  const selectedBreakdown = selectedListing
    ? calculateSaleBreakdown(currentListingAmount(selectedListing), Boolean(selectedPartner))
    : null;
  ...
  function submitListing(event: FormEvent<HTMLFormElement>) {
    ...
    const auctionStartAmount = Number(listingDraft.auctionStartAmount || amount);
    const auctionDays = Math.max(1, Math.floor(Number(listingDraft.auctionDays || 7)));
    const auctionStartsAt = new Date();
    const auctionEndsAt = new Date(auctionStartsAt.getTime() + auctionDays * 24 * 60 * 60 * 1000);
    const quantityAvailable = listingDraft.tokenStandard === "erc1155"
      ? Math.max(1, Math.floor(Number(listingDraft.editionSize || 1)))
      : 1;
    ...
    const listing: Listing = {
      ...
      collectionMode: listingDraft.collectionMode,
      tokenStandard: listingDraft.tokenStandard,
      quantityAvailable,
      saleMechanism: listingDraft.saleMechanism,
      auctionStartAmount: listingDraft.saleMechanism === "dutch" ? Math.max(auctionStartAmount, amount) : undefined,
      auctionStartsAt: listingDraft.saleMechanism === "dutch" ? auctionStartsAt.toISOString() : undefined,
      auctionEndsAt: listingDraft.saleMechanism === "dutch" ? auctionEndsAt.toISOString() : undefined,
      acceptsOffers: listingDraft.acceptsOffers || listingDraft.saleMechanism === "offer",
    ...
    setView("checkout");
    setNotice(`${collectionModeLabel(listing.collectionMode)} ${standardLabel(listing.tokenStandard)} listing staged with fees applied to ${saleMechanismLabel(listing.saleMechanism).toLowerCase()}.`);
  }
  ...
  async function simulateSale() {
    ...
    let transactionResult: SuperStoresTransactionResult;
    try {
      transactionResult = await fetch("/api/transactions/settle-and-release", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          saleId,
          listingId: selectedListing.id,
          marketplaceListingId: selectedListing.marketplaceListingId,
          buyerWallet: wallet.address,
          sellerWallet: selectedListing.sellerWallet,
          platformTreasuryWallet: settings.treasuryWallet,
          referrerWallet: selectedPartner?.wallet,
          referrerCode: selectedPartner ? activeReferrer : undefined,
          buyerCurrency,
          ...
          collectionMode: selectedListing.collectionMode,
          tokenStandard: selectedListing.tokenStandard,
          saleMechanism: selectedListing.saleMechanism,
          quantity: 1,
          environment
        })
      }).then(async (response) => {
        const data = await response.json() as SuperStoresTransactionResult;
        if (!response.ok || !data.ok) {
          throw new Error(data.error || "SuperStores transaction failed.");
        }
        return data;
      });
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "SuperStores transaction failed.");
      setIsSettling(false);
      return;
    }
    setIsSettling(false);
    ...
    const sale: Sale = {
      ...
      tokenStandard: selectedListing.tokenStandard,
      saleMechanism: selectedListing.saleMechanism,
      paymentChain: transactionResult.paymentChain,
      paymentChainId: transactionResult.paymentChainId,
      collectiblesChain: transactionResult.collectiblesChain,
      keeperHubExecutionIds: transactionResult.keeperHub.executionIds,
      marketplaceReleaseTxHash: transactionResult.release.txHash,
      settlementStatus: transactionResult.keeperHub.status,
      releaseStatus: transactionResult.release.status,
      keeperHubRoute: `${buyerCurrency}->${selectedListing.currency}->seller/platform${selectedPartner ? "/referrer" : ""}`,
      createdAt: new Date().toISOString()
    };
    ...
    setNotice(`${transactionResult.keeperHub.network} KeeperHub settlement ${transactionResult.keeperHub.status}; 0G release ${transactionResult.release.status}.`);
  }
...
function SellerView(props: {
  ...
  const auctionStartAmount = Number(props.draft.auctionStartAmount || amount);
  const displayAmount = props.draft.saleMechanism === "dutch" ? Math.max(auctionStartAmount, amount) : amount;
  const breakdown = calculateSaleBreakdown(displayAmount, true);
  return (
    <div className="view-stack">
      <section className="section-header">
        <div>
      ...
      <form className="seller-layout" onSubmit={props.onSubmit}>
        <div className="form-grid">
          ...
          <label className="field">
            ...
            <select
              value={props.draft.collectionMode}
              onChange={(event) => props.onDraft({ ...props.draft, collectionMode: event.target.value as CollectionMode })}
            >
              {COLLECTION_MODES.map((mode) => (
                <option key={mode} value={mode}>{collectionModeLabel(mode)}</option>
              ))}
            </select>
          </label>
          <label className="field">
            ...
            <select
              value={props.draft.tokenStandard}
              onChange={(event) => props.onDraft({ ...props.draft, tokenStandard: event.target.value as CollectibleStandard })}
            >
              {COLLECTIBLE_STANDARDS.map((standard) => (
                <option key={standard} value={standard}>{standardLabel(standard)}</option>
              ))}
            </select>
          </label>
          <label className="field">
            <span>Sale style</span>
            <select
              value={props.draft.saleMechanism}
              onChange={(event) => props.onDraft({ ...props.draft, saleMechanism: event.target.value as SaleMechanism })}
            >
              {SALE_MECHANISMS.map((mechanism) => (
                <option key={mechanism} value={mechanism}>{saleMechanismLabel(mechanism)}</option>
              ))}
            </select>
          </label>
          {props.draft.tokenStandard === "erc1155" && (
            <TextField
              label="Edition or license count"
              value={props.draft.editionSize}
              onChange={(editionSize) => props.onDraft({ ...props.draft, editionSize })}
            />
          )}
          {props.draft.saleMechanism === "dutch" && (
            <>
              <TextField
                label="Dutch auction start amount"
                value={props.draft.auctionStartAmount}
                onChange={(auctionStartAmount) => props.onDraft({ ...props.draft, auctionStartAmount })}
              />
              <TextField
                label="Auction duration days"
                value={props.draft.auctionDays}
                onChange={(auctionDays) => props.onDraft({ ...props.draft, auctionDays })}
              />
            </>
          )}
          <label className="field toggle-field">
            ...
            <button
              type="button"
              className={props.draft.acceptsOffers || props.draft.saleMechanism === "offer" ? "chip active" : "chip"}
              onClick={() => props.onDraft({ ...props.draft, acceptsOffers: !props.draft.acceptsOffers })}
            >
              {props.draft.acceptsOffers || props.draft.saleMechanism === "offer" ? "Accepted" : "Disabled"}
            </button>
          </label>
        ...
        <aside className="distribution-card">
          ...
          <div className="distribution-line">
            ...
            <strong>{collectionModeLabel(props.draft.collectionMode)} / {standardLabel(props.draft.tokenStandard)}</strong>
          </div>
          <div className="distribution-line">
            <span>Sale style</span>
            <strong>{saleMechanismLabel(props.draft.saleMechanism)}</strong>
          </div>
          <div className="distribution-line">
  ...
  return (
    <div className="checkout-layout">
      ...
      <section className="checkout-card">
        ...
        <div className="distribution-line">
          <span>Sale style</span>
          <strong>{saleMechanismLabel(props.listing.saleMechanism)}</strong>
        </div>
        <div className="distribution-line">
          <span>Collection mode</span>
          <strong>{collectionModeLabel(props.listing.collectionMode)} / {standardLabel(props.listing.tokenStandard)}</strong>
        </div>
        <div className="distribution-line">
        ...
        <div className="routing-preview">
          ...
          <p>{props.selectedPartner ? "10% platform, 10% referrer" : "20% platform, no referrer"} applies to this sale style; KeeperHub handles conversion internally; {props.listing.collectionMode === "database" ? "gasless database ownership is retained with ERC-compatible metadata" : `collectible anchoring on ${configuredCollectiblesChain}`}.</p>
        </div>
        <button type="button" onClick={props.onSimulateSale} disabled={props.isSettling}>
          <CircleDollarSign size={16} /> {props.isSettling ? "Settling..." : "Settle and release"}
        </button>
      </section>
      <section className="checkout-card">
        ...
        <pre>{JSON.stringify({
          ...
          referrerCode: props.selectedPartner ? props.activeReferrer : null,
          collectionMode: props.listing.collectionMode,
          tokenStandard: props.listing.tokenStandard,
          saleMechanism: props.listing.saleMechanism,
          paymentChain: configuredPaymentChain,
          paymentSettlement: "KeeperHub first, then 0G marketplace release",
        ...
        <div className="sales-list">
          {props.sales.slice(0, 3).map((sale) => (
            <div key={sale.id}>
              ...
              <span>{formatCurrency(sale.finalAmount, sale.settlementCurrency)} settled on {sale.paymentChain} / {sale.releaseStatus} / {sale.referrerCode || "no referrer"}</span>
            </div>
          ))}
        </div>
      </section>
...
function ListingCard({ listing, onSelect, compact = false }: { listing: Listing; onSelect: (listing: Listing) => void; compact?: boolean }) {
  const currentAmount = currentListingAmount(listing);
  const breakdown = calculateSaleBreakdown(currentAmount, true);
  return (
    <article className={compact ? "listing-card compact" : "listing-card"}>
      ...
      <div className="listing-body">
        ...
        <div className="metadata-list">
          <span>{collectionModeLabel(listing.collectionMode)}</span>
          <span>{standardLabel(listing.tokenStandard)}</span>
          <span>{saleMechanismLabel(listing.saleMechanism)}</span>
        ...
        <div className="listing-footer">
          <div>
            <span>{listing.saleMechanism === "dutch" ? "Current auction ask" : "Seller ask"}</span>
            <strong>{formatCurrency(currentAmount, listing.currency)}</strong>
          </div>
          <div>
            <span>Customer pays</span>
